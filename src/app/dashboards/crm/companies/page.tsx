    ...
    <AppLayout>
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold tracking-tight text-[#252f4a]">الشركات</h1>
            <p className="text-xs text-[#6b7177] mt-1">إدارة الشركات والعملاء المؤسسيين</p>
          </div>
                    ...
                    <tr key={c.id} className="hover:bg-[#f8f9fa] cursor-pointer transition-colors" onClick={() => { setSelected(c); setTab('info'); loadRelated(c.id); }}>
                      <td className="px-4 py-3 font-semibold text-[#252f4a]">
                        <span className="flex items-center gap-2">
                          <span className="w-7 h-7 rounded-lg bg-[#eaf3ff] text-[#3e97ff] flex items-center justify-center"><Building2 className="w-3.5 h-3.5" /></span>
                          {c.name}
                        </span>
                      </td>
                      <td className="px-4 py-3 text-xs text-[#4b5675]">{c.industry || '—'}</td>
                      ...
                      <td className="px-4 py-3 text-xs text-[#4b5675]">{c.city || '—'}</td>
                      ...
                      <td className="px-4 py-3 text-xs font-bold text-[#3e97ff]">{c._count?.contacts ?? '—'}</td>
                      <td className="px-4 py-3 text-xs text-[#6b7177]">{formatDate(c.createdAt)}</td>
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1" onClick={(e) => e.stopPropagation()}>
                          <Button variant="ghost" size="sm" onClick={() => openEdit(c)}><Pencil className="w-3.5 h-3.5 text-[#3e97ff]" /></Button>
                          <Button variant="ghost" size="sm" onClick={() => setDeleting(c)}><Trash2 className="w-3.5 h-3.5 text-[#d13b4c]" /></Button>
                        </div>
                      </td>
                    </tr>
                  ))}
      ...
      <Modal isOpen={!!deleting} onClose={() => setDeleting(null)} title="تأكيد الحذف" maxWidth="sm">
        <p className="text-sm text-[#4b5675]">هل أنت متأكد من حذف شركة <span className="font-bold text-[#252f4a]">{deleting?.name}</span>؟ لا يمكن التراجع عن هذا الإجراء.</p>
          ...
          <>
            {tab === 'info' && (
              <div className="p-4 bg-[#f8f9fa] rounded-xl grid grid-cols-2 gap-3 text-xs">
                <div><span className="text-[#9ca3af]">المجال:</span><p className="font-medium text-[#252f4a]">{selected?.industry || '—'}</p></div>
                <div><span className="text-[#9ca3af]">الحجم:</span><p className="font-medium text-[#252f4a]">{companySizeLabels[selected?.size]?.ar || '—'}</p></div>
                <div><span className="text-[#9ca3af]">المدينة:</span><p className="font-medium text-[#252f4a]">{selected?.city || '—'}، {selected?.country || ''}</p></div>
                <div><span className="text-[#9ca3af]">الموقع:</span><p className="font-medium text-[#252f4a]" dir="ltr">{selected?.website || '—'}</p></div>
              ...
              <div className="divide-y divide-[#eef0f3]">
                {related.contacts.map((c) => (
                  <div key={c.id} className="py-2.5 flex items-center justify-between text-xs">
                    <span className="font-semibold text-[#252f4a]">{c.firstName} {c.lastName}</span>
                    <span className="text-[#6b7177] font-mono" dir="ltr">{c.phone}</span>
                  </div>
                ))}
              ...
              <div className="divide-y divide-[#eef0f3]">
                {related.invoices.map((inv) => (
                  <div key={inv.id} className="py-2.5 flex items-center justify-between text-xs">
                    <span className="font-semibold text-[#252f4a]">{inv.invoiceNumber}</span>
                    <span className="flex items-center gap-2">
                      <span className="font-bold text-[#252f4a]">{formatCurrency(inv.total, inv.currency)}</span>
