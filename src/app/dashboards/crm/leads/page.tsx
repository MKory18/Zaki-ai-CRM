    ...
    <AppLayout>
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold tracking-tight text-[#252f4a]">العملاء المحتملون</h1>
            <p className="text-xs text-[#6b7177] mt-1">تتبع العملاء المحتملين وتحويلهم إلى جهات اتصال</p>
          </div>
          ...
          <CardContent className="p-4 space-y-4">
            <div className="flex flex-col sm:flex-row gap-3">
              <div className="relative flex-1 max-w-md">
                ...
                <input type="text" placeholder="بحث بالاسم، الشركة، الهاتف..." value={search} onChange={(e) => setSearch(e.target.value)}
              ...
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-[#eef0f3] bg-white">
                  {loading ? (
                    ...
                    <tr key={l.id} className="hover:bg-[#f8f9fa] cursor-pointer transition-colors" onClick={() => { setSelected(l); loadRelated(l.id); }}>
                      <td className="px-4 py-3 font-semibold text-[#252f4a]">{l.firstName} {l.lastName}</td>
                      <td className="px-4 py-3 text-xs text-[#4b5675]">{l.companyName || '—'}</td>
                      ...
                      <td className="px-4 py-3">
                        <span className={`text-xs font-bold ${l.score >= 70 ? 'text-[#25b865]' : l.score >= 40 ? 'text-[#e49e3d]' : 'text-[#9ca3af]'}`}>{l.score ?? 0}</span>
                      </td>
                      <td className="px-4 py-3 text-xs text-[#4b5675]">{l.assignedTo?.name || '—'}</td>
                      <td className="px-4 py-3 text-xs text-[#6b7177]">{formatDate(l.createdAt)}</td>
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1" onClick={(e) => e.stopPropagation()}>
                          {l.status !== 'CONVERTED' && (
                            <Button variant="ghost" size="sm" onClick={() => openConvert(l)} title="تحويل">
                              <ArrowLeftRight className="w-3.5 h-3.5 text-[#25b865]" />
                            </Button>
                          )}
                          <Button variant="ghost" size="sm" onClick={() => openEdit(l)}><Pencil className="w-3.5 h-3.5 text-[#3e97ff]" /></Button>
                          <Button variant="ghost" size="sm" onClick={() => setDeleting(l)}><Trash2 className="w-3.5 h-3.5 text-[#d13b4c]" /></Button>
                        </div>
                      </td>
                    </tr>
                  ))}
      ...
      <Modal isOpen={!!converting} onClose={() => setConverting(null)} title="تحويل العميل المحتمل" subtitle={converting ? `${converting.firstName} ${converting.lastName}` : ''} maxWidth="md">
        {convertResult ? (
          <div className="space-y-4 text-center py-4">
            <CheckCircle2 className="w-12 h-12 text-[#25b865] mx-auto" />
            <p className="text-sm font-semibold text-[#252f4a]">تم التحويل بنجاح!</p>
          ...
          <div className="space-y-4">
            {convertError && <div className="p-3 bg-[#fbeeef] border border-[#f4d7da] text-[#d13b4c] text-xs rounded-lg">{convertError}</div>}
            <p className="text-sm text-[#4b5675]">سيتم إنشاء جهة اتصال جديدة من هذا العميل المحتمل. يمكنك اختيارياً إنشاء صفقة مرتبطة:</p>
            <div className="grid grid-cols-2 gap-3">
      ...
      <Modal isOpen={!!deleting} onClose={() => setDeleting(null)} title="تأكيد الحذف" maxWidth="sm">
        <p className="text-sm text-[#4b5675]">هل أنت متأكد من حذف العميل المحتمل <span className="font-bold text-[#252f4a]">{deleting?.firstName} {deleting?.lastName}</span>؟</p>
        ...
        {relatedLoading ? <p className="py-8 text-center text-xs text-[#9ca3af]">جارٍ التحميل...</p> : (
          <div className="space-y-4">
            <div className="p-4 bg-[#f8f9fa] rounded-xl grid grid-cols-2 gap-3 text-xs">
              <div><span className="text-[#9ca3af]">الهاتف:</span><p className="font-mono font-bold text-[#252f4a]" dir="ltr">{selected?.phone}</p></div>
              <div><span className="text-[#9ca3af]">البريد:</span><p className="font-medium text-[#252f4a]" dir="ltr">{selected?.email || '—'}</p></div>
              <div><span className="text-[#9ca3af]">الشركة:</span><p className="font-medium text-[#252f4a]">{selected?.companyName || '—'}</p></div>
              <div><span className="text-[#9ca3af]">الدرجة:</span><p className="font-bold text-[#252f4a]">{selected?.score ?? 0}</p></div>
            </div>
            <h4 className="text-xs font-bold uppercase tracking-wider text-[#4b5675]">الأنشطة</h4>
