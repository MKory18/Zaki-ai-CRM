    ...
    <AppLayout>
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold tracking-tight text-[#252f4a]">لوحة CRM</h1>
            ...
            {quickActions.map((a) => (
              <Link key={a.href} href={a.href}>
                <Button size="sm" variant="outline" className="flex items-center gap-1.5 rtl:space-x-reverse">
                  <a.icon className="w-3.5 h-3.5 text-[#3e97ff]" />
                  <span>{a.label}</span>
                </Button>
              </Link>
            ))}
                    ...
                    {(data?.topOpenDeals || []).slice(0, 6).map((d: any) => (
                      <div key={d.id} className="py-2.5 px-2 flex items-center justify-between gap-2">
                        ...
                        <div className="text-right shrink-0">
                          <p className="text-sm font-bold text-[#252f4a]">{formatCurrency(d.value, d.currency)}</p>
                    ...
                    {(data?.upcomingTasks || []).slice(0, 6).map((task: any) => (
                      <div key={task.id} className="py-2.5 px-2 flex items-center justify-between gap-2">
                        <div className="min-w-0 flex items-center gap-2">
                          <CheckCircle2 className="w-4 h-4 text-[#3e97ff] shrink-0" />
                          <p className="text-sm font-medium text-[#252f4a] truncate">{task.title}</p>
                        </div>
                  ...
                  <div className="space-y-3 border-r border-[#eef0f3] pr-4">
                    {(data?.recentActivities || []).slice(0, 8).map((a: any) => (
                      <div key={a.id} className="relative">
                        <span className="absolute -right-[22px] top-1 w-2 h-2 rounded-full bg-[#3e97ff]" />
                        ...
                        <p className="text-xs text-[#252f4a] font-medium mt-1">{a.subject}</p>
                        {a.description && <p className="text-[11px] text-[#6b7177] line-clamp-2">{a.description}</p>}
                      </div>
                    ))}
