    ...
    <AppLayout>
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold tracking-tight text-[#252f4a]">المنتجات</h1>
            <p className="text-xs text-[#6b7177] mt-1">عرض المنتجات من متجرك للربط بالصفقات والعروض (قراءة فقط)</p>
          </div>
          <Link href="/products">
                    ...
                    <tr key={p.id} className="hover:bg-[#f8f9fa] transition-colors">
                      <td className="px-4 py-3">
                        <span className="flex items-center gap-3">
                          ...
                          <span className="font-semibold text-[#252f4a]">{p.name}</span>
                        </span>
                      </td>
                      <td className="px-4 py-3 text-xs font-mono text-[#4b5675]" dir="ltr">{p.sku || '—'}</td>
                      <td className="px-4 py-3 text-xs font-bold text-[#252f4a]">{formatCurrency(p.basePrice, p.currency || 'USD')}</td>
                      <td className="px-4 py-3">
