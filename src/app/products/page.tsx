          ...
          <CardContent className="pt-4">
            ...
            <div className="relative max-w-md mb-4">
              ...
              <input
                ...
                className="w-full ps-9 pe-4 py-2.5 text-xs bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-red-500/20 focus:border-red-500"
              />
              {search && (
                <button
                  onClick={() => setSearch('')}
                  className="absolute end-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-700 cursor-pointer"
                >
                  <X className="w-4 h-4" />
                </button>
              )}
              <span className="absolute -bottom-5 start-1 text-[10px] text-slate-400">
                  ...
                  {filteredProducts.map((p) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-6 py-3">
                          <span className="font-bold text-slate-900 block">{productName(p, locale)}</span>
                        ...
                        <td className="px-6 py-3 text-right rtl:text-left">
                          <div className="flex items-center justify-end gap-1.5" onClick={(e) => e.stopPropagation()}>
                            ...
                            <Button
                              ...
                              variant="secondary"
                              onClick={() => openEdit(p)}
                              className="p-2 bg-blue-50 text-blue-700 border-blue-200 hover:bg-blue-100"
                              title="تعديل المنتج"
                            >
      ...
      >
        <form onSubmit={handleCreateProduct} className="space-y-4">
          ...
          <div className="border border-slate-200 rounded-xl p-4 bg-slate-50/50 space-y-3">
            <h4 className="text-xs font-bold uppercase tracking-wider text-slate-700">1. المعلومات الأساسية</h4>
            <Input
              label="اسم المنتج *"
          ...
          <div className="border border-slate-200 rounded-xl p-4 bg-slate-50/50 space-y-3">
            ...
            {selectedFiles.length === 0 ? (
              <div className="border-2 border-dashed border-slate-300 rounded-xl p-6 text-center text-xs text-slate-400">
                ...
                {selectedFiles.map((file, idx) => (
                  <div
                    ...
                    className={`relative group rounded-xl overflow-hidden border-2 ${
                      primaryIndex === idx ? 'border-red-500' : 'border-slate-200'
                    }`}
                  >
                    ...
                    />
                    {primaryIndex === idx && (
                      <span className="absolute top-1 left-1 rtl:left-auto rtl:right-1 bg-red-600 text-white text-[9px] font-bold px-1.5 py-0.5 rounded flex items-center space-x-0.5">
                    ...
                    <button
                      ...
                      className="absolute top-1 right-1 rtl:right-auto rtl:left-1 bg-white/90 text-red-600 rounded-full p-0.5 opacity-0 group-hover:opacity-100 transition-opacity cursor-pointer"
                    >
                      <X className="w-3 h-3" />
                    </button>
                    {primaryIndex !== idx && (
      ...
      >
        <form onSubmit={handleEditSave} className="space-y-4">
          ...
          <div className="flex justify-end space-x-2 rtl:space-x-reverse pt-1">
            ...
            <Button type="submit" loading={editLoading} className="bg-blue-600 hover:bg-blue-700">
