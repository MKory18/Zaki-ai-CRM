    ...
    <div className="col-span-full rounded-lg bg-[var(--sys-surface)] px-3 py-2.5">
      ...
      <div className="flex flex-wrap items-end gap-2">
        <label className="block">
          <span className="mb-0.5 block text-caption text-[var(--sys-muted-foreground)]">يبدأ</span>
          <input
            type="time"
            value={start}
            disabled={!canEdit}
        ...
        <label className="block">
          <span className="mb-0.5 block text-caption text-[var(--sys-muted-foreground)]">يسلّم</span>
          <input
            type="time"
            value={end}
            disabled={!canEdit}
      ...
      <div className="mt-2">
        <span className="mb-1 block text-caption text-[var(--sys-muted-foreground)]">
          أيام الراحة{' '}
        ...
        <div className="flex flex-wrap gap-1">
          {DAYS.map((d) => (
            <button
              ...
              className={`rounded-lg border px-1.5 py-0.5 text-caption ${
                rest?.includes(d.n)
          ...
          {canEdit && rest !== null && (
            <button
              ...
              className="rounded-lg px-1.5 py-0.5 text-caption text-[var(--sys-muted)] hover:text-[var(--sys-primary)]"
            >
              أعد عطلة البلد
            </button>
          )}
      ...
      {inverted && <p className="mt-1 text-caption text-[var(--sys-destructive)]">وقت التسليم يجب أن يكون بعد وقت البدء.</p>}
      {!inverted && !start && !end && rest === null && (
        <p className="mt-1 text-caption text-[var(--sys-muted)]">فارغ = ساعات البلد وعطلته. التأخير يُقاس بما هنا.</p>
      )}
